...
import { splitBillService } from '@/features/split-bill/services/split-bill.service';
...
export function SplitBillJoinPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await splitBillService.joinSession({
        sessionCode: normalizedSessionCode,
        displayName: displayName.trim(),
      });

      if (!response.success || !response.data) {
        toast.error(response.message || 'Gagal join ke session split bill');
        return;
      }

      toast.success('Berhasil join ke session split bill');
      router.push(
        `/split-bill/session/${encodeURIComponent(response.data.sessionCode)}?joinedAs=${encodeURIComponent(displayName.trim())}`,
      );
    } catch (error: any) {
      toast.error(error?.message || 'Gagal join ke session split bill');
    } finally {
        ...
        <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <Link href="/login" className="inline-flex items-center gap-3 text-slate-800">
            ...
            <div>
              ...
              <p className="text-lg font-bold">Join Split Bill</p>
            </div>
          </Link>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="overflow-hidden rounded-[32px] bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.24),_transparent_35%),linear-gradient(135deg,#0f172a_0%,#1d4ed8_52%,#38bdf8_100%)] p-7 text-white shadow-[0_28px_60px_rgba(29,78,216,0.24)] sm:p-10">
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-white/82">
              Owner session tinggal bagikan kodenya. Kamu isi nama, masuk ke
              halaman detail, lalu lihat pembagian tagihan yang sedang aktif.
            </p>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-3xl border border-white/15 bg-white/12 p-4 backdrop-blur">
                <ArrowRight size={20} />
                <p className="mt-3 text-sm font-semibold">Lihat detail</p>
                <p className="mt-1 text-xs leading-5 text-white/70">
                  Setelah join, kamu langsung masuk ke halaman session.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_24px_50px_rgba(15,23,42,0.08)] sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder="SPLIT_BILL_OSISI_2026"
            ...
            <div className="mt-6 rounded-3xl bg-slate-50 p-4">
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                ...
                buat room baru dari dashboard split bill.
              </p>
            </div>
          </section>
        </div>
